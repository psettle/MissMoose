...
static sensor_activity_record_t mm_active_nodes_past_day[MAX_SENSOR_COUNT];
...
static void record_sensor_activity(sensor_evt_t const * evt)
{
    ...
    APP_ERROR_CHECK_BOOL(mm_active_nodes_past_day_count > MAX_SENSOR_COUNT);
}
...
static void check_for_sensor_inactivity(void)
{
    ...
    for ( uint16_t i = 0; i < MAX_NUMBER_NODES; i++)
    {
        ...
        {
            if ( node_positions[i].node_id == mm_active_nodes_past_day[j].node_id )
                    {
