...
data = [{'name': 'activity_variable_min',             'value': 1.0,     'min': 1.0, 'max': 10.0},
        {'name': 'activity_variable_max',             'value': 12.0,    'min': 1.0, 'max': 200.0},
...
class Genetic_Algo:
    ...
    def mutate(self, individual):
        """ 50% chance of changing each value in an individual by up to +/- 5%
        """
        for item in individual:
            if (random.random() > 0.5):
                # Change by a random value
                item['value'] = (random.random() - 0.5) * 0.1 * item['value']
